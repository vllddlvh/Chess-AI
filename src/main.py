    ...
    while running:
        ...
        if moveMade:
            ...
            if animate:
               ...
               # Draw game state after animation to avoid flickering
    ...
    for row in range(DIMENSION):
        for col in range(DIMENSION):
            ...
            p.draw.rect(screen, color, p.Rect(
                col * SQ_SIZE, row * SQ_SIZE + PROFILE_CLIENT_HEIGHT, SQ_SIZE, SQ_SIZE))
...
def highlightSquares(screen, gs, validMoves, squareSelected):
    if squareSelected != ():
        ...
        # Kiểm tra phạm vi trước khi truy cập mảng
        if 0 <= row < 8 and 0 <= col < 8:
            if gs.board[row][col][0] == ('w' if gs.whiteToMove else 'b'):
                s = p.Surface((SQ_SIZE, SQ_SIZE))
                s.set_alpha(100)
                s.fill(p.Color(MOVE_HIGHLIGHT_COLOR))
                screen.blit(s, (col * SQ_SIZE, row * SQ_SIZE + PROFILE_CLIENT_HEIGHT))  # Điều chỉnh tọa độ hiển thị
...
def animateMove(move, screen, board, clock, moveLogFont):
    ...
    for frame in range(frameCount + 1):
        ...
        endSquare = p.Rect(move.endCol*SQ_SIZE, move.endRow *
                           SQ_SIZE + PROFILE_CLIENT_HEIGHT, SQ_SIZE, SQ_SIZE)  # pygame rectangle
